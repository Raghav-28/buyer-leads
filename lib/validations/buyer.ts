...
// Partial schema for updates that maintains validation rules for provided fields
export const buyerUpdateSchema = z
  .object({
    ...
    email: z.string().email("Invalid email format").optional().or(z.literal("")),
    phone: z.string().min(10, "Phone number must be at least 10 digits").max(15, "Phone number must be at most 15 digits").optional(),
    ...
    budgetMin: z.number().min(0, "Budget must be non-negative").optional(),
    budgetMax: z.number().min(0, "Budget must be non-negative").optional(),
    timeline: z.enum(["M0_3m", "M3_6m", "MoreThan6m", "Exploring"]).optional(),
